...
// Read
$stmt = $pdo->query("SELECT * FROM foto WHERE userId = $userId");
$photos = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Read Album IDs
$stmtAlbums = $pdo->query("SELECT AlbumID, NamaAlbum FROM album WHERE userId = $userId");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
   ...
   $stmt = $pdo->prepare("UPDATE foto SET JudulFoto=?, DeskripsiFoto=?  WHERE FotoID=?");
   $stmt->execute([$JudulFoto, $DeskripsiFoto, $FotoID]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
   ...
   echo "<script>
               alert('Successfully delete photos!');
            </script>";
   if (!$stmt) {
   } else {
      echo "<script>
